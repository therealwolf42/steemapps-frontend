...
const actions = {
  async submitApp({ state, commit }, payload) {
    try {
      ...
      let { name, accounts, app_type, link, category, custom_jsons, custom_json, description, short_description, image, status, social, product_screenshot } = payload
      ...
      let response = await this.$axios.post('apps/submit/new',  {
        name, accounts, app_type, link, category, custom_jsons, description, short_description, image, status, social, product_screenshot
      })
      return response
    } catch (error) {
